...
def upload(conf, section):
    ...
    try:
        ...
        logging.info('Upload file SUCCESSFUL %s ' % datetime.datetime.now())
        print
        'Beginning to upload file to %s  %s ' % (host, datetime.datetime.now())
        ...
        logging.error('host: <' + host + '> connect error!' + e)
...
def select_section(conf_file_name):
    ...
    conf.read(conf_file_name, encoding="utf-8")
    ...
    while choose_section_tag:
        sec_index = input('please choose one item default [0]:')
